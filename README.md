> ## 本仓库说明（fork）
>
> 本仓库基于 [lnkiai/m3e-canvas](https://github.com/lnkiai/m3e-canvas) 修改而成。
> 在原版「浏览器内绘制 Material 3 Expressive 线框图并生成 vibe-coding 提示词」的基础上，
> 新增多项目支持与本地持久化、多标签页编辑权接管、项目管理界面、多语言文案、
> AI 服务商扩展（含小米 MiMo）以及 AI 草图意图感知 / 定向修改等，
...
一、支持小米 MiMo 最新 mimo-v2.6 系列模型，替换原有 v2.5 模型

**2026-09-17**

一、让AI来画新增"定向修改"模式：修改现有屏幕时只按 id 替换被点名的屏幕，其余屏幕零改动

**2026-09-15**

一、新增第 5 个 AI 服务商 Mimo，支持 mimo-v2.5 与 mimo-v2.5-pro 模型，支持 Token Plan / 按量付费双地址切换
...
| 文件          | 改动说明                                                                                                          |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------ |
| `lib/ai.ts` | Mimo 服务商模型升级：`mimo-v2.5` / `mimo-v2.5-pro` → `mimo-v2.6` / `mimo-v2.6-pro`，默认模型改为 `mimo-v2.6`（界面模型按钮与接口调用 `model` 参数同步生效） |
| `README.md` | 顶部说明补全新增功能梗概；更新日志与文件改动清单新增 2026-09-23 批次                                                                              |

**2026-09-17（让AI来画定向修改）**

**修改文件**

| 文件               | 改动说明                                                                                                                                                                |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib/ai.ts`      | 新增 `editInPlace`：`{"mode":"edit"}` 回复按 frame id 定向替换被点名屏幕（程序端强制保留原坐标，屏幕上的组整体替换），未点名屏幕与组不经模型之手；`applyDraft` 增加 edit 分支；`draftDesign` 提示词改为 add / edit / replace 三路意图，replace 仅用于主题/标题/删除屏幕等全局改动 |
| `lib/ai.test.ts` | 新增 edit 模式单元测试（4 例：按 id 替换并保位、省略组即删除、未知 id 报错、空 frames 报错）                                                                                                        |
...
| 文件                             | 改动说明                                                                                                       |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------- |
| `lib/persist/opfs.ts`          | OPFS 存储实现：`projects/<id>.json` 主文件 + `<id>.json.bak1~5` 节流滚动备份 + `<id>.thumb`（webp dataURL 文本）             |
| `lib/persist/projects.ts`      | 多项目索引纯函数（脱离浏览器可测）：`withProject` / `withoutProject` / `openedProject` / `renamedProject` / `sortedProjects` |
| `lib/persist/lock.ts`          | BroadcastChannel 接管协议：`ask-release` / `released`（1.2s 超时）协调多标签页编辑权                                         |
| `components/ProjectMenu.tsx`   | 项目切换气泡组件：当前项目名 + ▾，下拉列出全部项目（懒加载缩略图）、新建 / 重命名 / 删除 / 切换，只读时提供“接管编辑权”入口                                      |
| `lib/persist/projects.test.ts` | 多项目索引纯函数的单元测试                                                                                              |
| `lib/persist/persist.test.ts`  | PersistStore 多项目行为的单元测试                                                                                    |

**修改文件**

| 文件                        | 改动说明                                                                                                                                                                                                                                                                                             |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `lib/persist/index.ts`    | 重写 `PersistStore` 支持多项目：`init` / `openProject` / `createProject` / `renameProject` / `deleteProject` / `getThumb` / `setThumbProvider`；存储目标优先级 file > disk(OPFS) > local；新增 `subscribeProjects` 订阅与 `lang` / `setLang` / `translateAutoNames`（默认名本地化）；`init` 时把旧版 localStorage 画布迁移为首个项目；编辑权接管回调 |
| `lib/persist/types.ts`    | `SaveTarget` 增加 `"disk"`（OPFS）枚举值                                                                                                                                                                                                                                                                |
| `lib/persist/fsFile.ts`   | `FileAdapter` 增加 `setName` / `name`：每个项目一个 `.json`，重命名时同步改文件名                                                                                                                                                                                                                                    |
| `app/page.tsx`            | 接入持久化：`subscribeProjects` 订阅刷新 UI；`captureThumb`（toCanvas→160px webp，15s 节流）生成缩略图；`blankDoc` 新建空白画布；响应编辑权接管（只读横幅 + “接管”按钮）；`localStorage` 旧画布迁移；重命名 / 删除改用应用内对话框（`openRename`/`confirmRename`/`openDelete`/`confirmDelete`）；快捷键守卫在对话框打开时不再触发全局快捷键                                                |
| `components/Toolbar.tsx`  | 接入 `project` 系列 props（`project` / `onCreateProject` / `onSwitchProject` / `onRenameProject` / `onDeleteProject` 等），渲染项目切换 `ProjectChip`                                                                                                                                                          |
...
- **Optional AI helper** – bring your own key (OpenAI, Claude, Gemini, DeepSeek or Mimo) and let the model write a part's behavior note or a screen's description, in your language. Each rewrite can be undone. The key stays in your browser and the request goes straight to the provider; there is no server in between.
...
- **AI 補助（任意）** – 自分のキー（OpenAI、Claude、Gemini、DeepSeek、Mimo）を入れると、部品の動作や画面の説明を UI の言語で書いてもらえます。書き換えは元に戻せます。キーはブラウザ内にだけ保存され、リクエストはプロバイダへ直接送られます（間にサーバーはありません）。
...
- **AI 辅助（可选）** – 填入自己的密钥（OpenAI、Claude、Gemini、DeepSeek 或 Mimo），让模型用界面语言写出组件的行为或屏幕的说明。每次改写都可以撤销。密钥只保存在浏览器中，请求直接发送给服务商，中间没有服务器。
...
- **AI 도우미(선택)** – 자신의 키(OpenAI, Claude, Gemini, DeepSeek, Mimo)를 넣으면 부품의 동작이나 화면 설명을 UI 언어로 써 줍니다. 고쳐 쓴 내용은 되돌릴 수 있습니다. 키는 브라우저에만 저장되고 요청은 제공업체로 직접 전송됩니다(중간 서버 없음).
